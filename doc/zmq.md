...
The Leprechain Core daemon can be configured to act as a trusted "border
router", implementing the leprechain wire protocol and relay, making
...
The ZeroMQ feature in Leprechain Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building leprechaind:
...
For instance:
    ...
    $ leprechaind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/leprechaind.tx.raw \
...
These options can also be provided in leprechain.conf.
...
From the perspective of leprechaind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into leprechaind directly. Furthermore, no information is
...
using. Leprechaind appends an up-counting sequence number to each
