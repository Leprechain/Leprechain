...
const char * const BITCOIN_CONF_FILENAME = "leprechain.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //leprechain-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Leprechain
    // macOS: ~/Library/Application Support/Leprechain
    // Unix-like: ~/.leprechain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Leprechain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Leprechain";
#else
    // Unix-like
    return pathRet / ".leprechain";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty leprechain.conf if it does not exist
