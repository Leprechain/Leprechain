...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 LPC irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on leprechaincentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Leprechain features */
    { "leprechain",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "leprechain",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "leprechain",               "gobject",                &gobject,                {} },
    { "leprechain",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
