...
impl PrivateKey {
    ...
    pub fn generate_leprechain(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "leprechain_helpers")]
    pub fn generate_leprechain_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_leprechain(rng))
            .collect()
    }
